...
class PolicyLedger extends Controller {
    ...
    public function update($id) {
		...
		unset($rules['motor_engine']);
       	
        //dd($inputs);
       	 
        $validator = Validator::make($inputs, $rules);
        if($validator->fails()) {
			Session::flash('error', 'Please check the form below for error(s).!');
		...
		if(!empty($check)) {
			...
			foreach($check as $key => $value) {
				...
				if(empty($value['value'])) {
					...
					$check[$key]['value'] = (float)$value['value'];
				}
				
				if(empty($value['income'])) {
					$check[$key]['income'] = 0;
				} else {
					$check[$key]['income'] = (float)$value['income'];
				}
				...
				if(empty($value['premium'])) {
					...
					$check[$key]['premium'] = (float)$value['premium'];
				}
				
				
				if(empty($value['net_premium'])) {
					$check[$key]['net_premium'] = 0;
				} else {
					$check[$key]['net_premium'] = (float)$value['net_premium'];
				}
				
				if(empty($value['commission'])) {
					$check[$key]['commission'] = 0;
				} else {
					$check[$key]['commission'] = (float) $value['commission'];
				}
			}
		}
		//echo"<pre>";print_r($check->toArray());echo"</pre>";
		//dd($check->toArray());
